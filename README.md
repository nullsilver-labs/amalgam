...
- Edit a sent message: the pencil under it turns the bubble into a field, and what you send goes beside the original as a branch of its own, quoting the same cards. Nothing is overwritten.
- Statistics, when Settings › Chat asks for them: under your message the size of the request that answered it, under the model's the reply's tokens, its speed and its time. Counts are the provider's own where it reports them (OpenAI-style servers are asked to count; Anthropic counts in any case) and marked with a tilde as estimates where it does not. Settings › Usage totals them by period and by model.
- A chat's options, from the sliders mark before the model: thinking on or off for this chat alone, defaulting to Settings › Chat, and honest about what that does for the model in hand — Claude models are asked to think; an OpenAI-style provider takes no such flag, so only the reply's headroom changes. The same plate names the model's declared window, or says it is not declared and that the instance budget applies.
- Reading your own corpus library: search it from the composer's "+", attach up to five cards to a message, see them quoted with attribution that survives a reload, and be told before excerpts leave this machine. Read-only, server-to-server, only when you ask.
...
- Several chats open at once. The open chats sit in a strip above the plate, lined up with its edge, from the first chat on; New chat from inside a chat opens a tab of its own (a blank tab is reused); ⌘-click, middle-click or a row's menu opens a chat in a tab of its own. Open tabs survive a reload of the browser tab; a response goes on being written on the server when its tab closes, and is whole when the chat is opened again. On a phone the open chats head the Chats panel instead.
...
**Not implemented yet:** attachments of your own, ingestion or indexing of anything, vector search inside amalgam, automatic retrieval, web search, images/audio/video, voice, model-account OAuth, agents/MCP, multi-user accounts, document editor, or UI-based connection editing. The corpus connector reads cards you pick; it is not a knowledge base of amalgam's own.
...
The server caps input at 16,000 characters and context at the token budget set in Settings (32,000 by default), lowered to a declared model window less the reply's reservation of up to 4,096 tokens — plus 16,384 more for thinking while Settings › Chat › Thinking is on; at most the 101 nearest messages of the branch being read are considered. Tokens are **estimated from characters on the conservative side**, not counted by the model's tokenizer, so a model whose window is not declared can still reject an oversized request. Output is capped at 200,000 characters and each response at the time limit in Settings › Chat (ten minutes by default; some models think for longer, and nobody has to hold a connection open for it). Failed/partial assistant output is excluded from subsequent model context.

A response belongs to the server once it is started, not to the request that asked for it: the browser only listens. Closing the page, closing the tab or losing the connection does not stop it; the Stop button does, and keeps the text received so far. Every event is numbered and kept for the response's lifetime, so a tab that lost its connection asks for what came after the last event it heard, and a conversation opened while its response is still being written — after a reload, in another tab — joins from a snapshot of what has been said. Content checkpoints occur about once per second while deltas arrive. A crash can lose the most recent uncheckpointed delta, never silently invent completion. On restart, unfinished responses become `interrupted`, and nothing is resubmitted. A ghost chat's response is the exception: written nowhere, it ends with its request. Only one app process/replica is supported. A response is given the minutes Settings › Chat allows, ten by default.
...
The corpus connector adds one nullable column to `messages` on the same start (`schema_version` 3). It stays dormant until you set `CORPUS_BASE_URL` and `CORPUS_TOKEN`: without both, the library entry under the composer's "+" is disabled and no request is made to corpus.
