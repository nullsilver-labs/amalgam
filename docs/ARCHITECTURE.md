...
- `src/routes/api/chat/+server.ts`: transactionally accepts a user message + assistant placeholder before emitting start; checkpoints partial text and records terminal status. With `ghost: true` it writes nothing at all: the request carries the chat's own history, the context is assembled from it exactly as from rows, and the same streaming loop runs with no persistence hooks. The route reads bodies up to 2 MiB for that reason (compose sets `BODY_SIZE_LIMIT` to match).
